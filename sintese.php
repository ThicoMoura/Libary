...
<html lang="pt-BR">
  ...
  <div class="row">
     ...
     <!--Col s12 = Define o limite de Col-->
      ...
      <form action="insert_sintese.php?id=<?php echo $livroid;?>" method="post" enctype="multipart/form-data" class="col s12" id="teste">
        <div class="row">

  <!--Input-field = Define a class input-->

        <div class="input-field col s6">
          <input placeholder="Digite aqui o nome da sua Sintese" id="nomesi" type="text" class="validate" name="nomesi" required>
          <label for="nomesi">Nome</label>
        </div>

        <div class="input-field col s6">
          <input id="livro" type="text" name="nomesintese" value="<?php 
          $sql = "SELECT titulo FROM livro WHERE livroid = ".$livroid;

          $stmt = DB::prepare($sql);
          $stmt ->execute();
          foreach ($stmt->fetchALL(PDO::FETCH_ASSOC) as $value){echo utf8_encode($value['titulo']);}
          ?>" disabled class="disabled">
          <label for="nomesintese">Livro</label>
        </div>
